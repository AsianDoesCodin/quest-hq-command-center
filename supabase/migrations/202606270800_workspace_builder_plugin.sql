...
as $$
  select case
    ...
    when permission like 'underwriter.%' then 'underwriter'
    when permission like 'files.%' then 'files'
    when permission like 'client_portals.%' then 'client_portal'
    when permission like 'workspaces.%' then 'workspace_builder'
...
insert into public.company_plugins (company_id, plugin_id, status, installed_by, installed_at, disabled_at, updated_at)
values ('lumen', 'workspace_builder', 'installed', null, now(), null, now())
on conflict (company_id, plugin_id) do update
